...
def get_next_stages(request, application_id):
    ...
    role_token = _normalized_role_token(request.user)
    is_admin = (
...
def _get_application_by_id(application_id, user=None):
    """
    Find an application by application_id (string PK / reference_no / ref_no).
    Returns the instance or None (filtered by user district if district-scoped role).
    """
    return WorkflowService.get_application_by_id(application_id, user=user)
