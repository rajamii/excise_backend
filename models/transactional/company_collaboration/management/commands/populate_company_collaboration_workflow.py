...
TRANSITIONS = [
    ('applicant_applied',         'permit_section',            {}),
...
class Command(BaseCommand):
    ...
    def _seed_workflow(self):
        ...
        # Delete existing transitions for this workflow to ensure they are updated with the correct conditions
    ...
    def _seed_fees(self):
        ...
        # Delete old fee entries
        deleted_count, _ = MasterFixedFee.objects.filter(fee_code__in=['COMP_COLLAB_APP', 'COMP_COLLAB_SEC']).delete()
        if deleted_count > 0:
            self.stdout.write(f'  Deleted {deleted_count} legacy/deprecated fee codes (COMP_COLLAB_APP, COMP_COLLAB_SEC)')
        ...
        for row in FEE_ROWS:
            ...
            if created:
                self.stdout.write(f'  Fee seeded: {obj.fee_code} = Rs.{obj.amount}')
            else:
