...
def _extract_request(*args, **kwargs):
    for arg in args:
        if hasattr(arg, "user") and (hasattr(arg, "query_params") or hasattr(arg, "GET")):
            return arg
        if hasattr(arg, "request") and hasattr(arg.request, "user"):
            return arg.request
...
def dashboard_counts_cache(namespace: str):
    ...
    def decorator(view_func):
        ...
        def wrapper(*args, **kwargs):
            ...
            query_params = getattr(request, "query_params", request.GET if hasattr(request, "GET") else {})
            has_cache_buster = any(k in query_params for k in BYPASS_CACHE_QUERY_PARAMS)
            if not has_cache_buster and request is not None:
            ...
            if 200 <= status_code < 300 and isinstance(response_data, (dict, list)) and request is not None:
