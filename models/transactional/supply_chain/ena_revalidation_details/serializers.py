...
class EnaRevalidationDetailSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        validity_days = self._get_revalidation_validity_period_days()
        data['validity_period_days'] = validity_days

        approval_anchor = self._resolve_revalidation_approval_anchor(instance, effective_status)
        if approval_anchor is not None:
    ...
    def _get_revalidation_validity_period_days(self) -> int:
        cached = getattr(self, '_cached_revalidation_validity_days', None)
        ...
        self._cached_revalidation_validity_days = resolved_days
        return resolved_days

    def _resolve_revalidation_approval_anchor(self, obj, effective_status):
        status_token = self._normalize_status_token(effective_status)
        is_approved = ('approv' in status_token) and ('reject' not in status_token)

        if is_approved:
            return (
                getattr(obj, 'updated_at', None)
                or getattr(obj, 'revalidation_date', None)
                or getattr(obj, 'created_at', None)
            )

        return (
            getattr(obj, 'revalidation_date', None)
            or getattr(obj, 'created_at', None)
        )
