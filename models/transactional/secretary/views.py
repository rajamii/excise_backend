...
def _get_factories_data(subcat_filter='', search_q=''):
    ...
    try:
        apps_qs = list(NewLicenseApplication.objects.select_related('license_category', 'license_sub_category', 'site_district').all())
    except Exception:
        apps_qs = []
    ...
    for app in apps_qs:
        try:
            ...
            cat_name = getattr(cat_obj, 'license_category', '') or ''
            subcat_name = getattr(subcat_obj, 'description', '') or ''
            ...
            est_name = (getattr(app, 'establishment_name', '') or getattr(app, 'company_name', '') or getattr(app, 'applicant_name', '') or f"Factory #{getattr(app, 'application_id', '')}").strip()
            comp_name = (getattr(app, 'company_name', '') or est_name).strip()
            applicant_name = (getattr(app, 'applicant_name', '') or 'Authorized Licensee').strip()

            if search_q:
                combined = f"{est_name} {getattr(app, 'application_id', '')} {comp_name} {applicant_name}".lower()
            ...
            district_obj = getattr(app, 'site_district', None)
            district_name = getattr(district_obj, 'district', None) or 'Gangtok'

            app_id = getattr(app, 'application_id', '') or ''
            matched_license = License.objects.filter(license_id__icontains=app_id).first() if app_id else None
            existing_no = getattr(app, 'existing_license_no', '') or ''
            lic_no = getattr(matched_license, 'license_id', None) if matched_license else (
                existing_no if (existing_no and len(existing_no.strip()) > 3) else f"LIC/{app_id}"
            )
            ...
            factories.append({
                ...
                'business_address': getattr(app, 'business_address', '') or f"{district_name}, East Sikkim",
                'mobile_number': getattr(app, 'mobile_number', '') or getattr(app, 'company_phone_number', '') or '9800001234',
...
def secretary_bulk_spirit_factories(request):
    ...
    try:
        ...
        logger.exception("Error in secretary_bulk_spirit_factories: %s", e)
        return Response(_to_json_safe({
            'count': 0,
            'factories': []
        }), status=200)
...
def secretary_bulk_spirit_summary(request):
    ...
    try:
        ...
        distilleries_count = sum(1 for f in factories if f.get('sub_category') == 'Distillery')
        breweries_count = sum(1 for f in factories if f.get('sub_category') == 'Brewery')
        total_stock_bl = sum(f.get('stock_bl', 0) for f in factories)
        total_requested_bl = sum(f.get('total_bl_requested', 0) for f in factories)
        total_dispatched_bl = sum(f.get('dispatched_bl', 0) for f in factories)
        total_requisitions = sum(f.get('total_requisitions_count', 0) for f in factories)
        ...
        logger.exception("Error in secretary_bulk_spirit_summary: %s", e)
        return Response(_to_json_safe({
            'total_units': 0,
